...
from src.tasks.heads.attention_reducer import AttentionReducer
from src.tasks.heads.mean_reducer import MeanReducer
from src.tasks.heads.max_reducer import MaxReducer
from src.tasks.heads.glu_reducer import GLUReducer
from src.tasks.heads.feature_applier import FeatureApplier, Feature
from src.models.layers.activations import build_activation, ActivationFn
...
class ClassificationHead(nn.Module):
    def __init__(
        ...
        if num_hidden_layers > 0:
            self.ff = nn.Sequential(
                linear_block(input_dim, ff_dim, dropout_p, activation_fn),
                *[
                    linear_block(ff_dim, ff_dim, dropout_p, activation_fn)
                    for _ in range(num_hidden_layers - 1)
                ],
                nn.Linear(ff_dim, output_dim),
            )
