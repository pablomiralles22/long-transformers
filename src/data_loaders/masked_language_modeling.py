...
class MLMDataset(Dataset):
    def __init__( self, root_dir: str):
        """
        Args:
            root_dir (string): Directory containing txt files
        """
...
class MLMCollatorFn:
    def __init__(self, max_len: int, mask_ratio: float=0.3, random_ratio: float=0.33):
...
class MLMDataModule(pl.LightningDataModule):
    ...
    def __init__(self, data_path: str, collator_config: dict, loader_config: dict):
        ...
        # load datasets
        self.trainval_dataset = MLMDataset(data_path)

        n = len(self.trainval_dataset)
        n_train = int(self.__TRAIN_RATIO * n)
        n_val = n - n_train

        self.train_dataset, self.val_dataset = torch.utils.data.random_split(
            self.trainval_dataset, [n_train, n_val]
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            **self.loader_config,
            collate_fn=MLMCollatorFn(**self.collator_config),
            shuffle=True,
        )
